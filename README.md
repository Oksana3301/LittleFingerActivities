...
Worksheet data is kept in 159 server-only category files and delivered through access-checked API routes on demand; fetched categories are cached in memory. Progress, strokes, favorites and stickers use versioned browser storage (`astra-workbook-v2`); previous family data remains in `little-world-v1`. Guest previews keep device-local history. Verified customer accounts have child profiles and revision-checked cloud workbooks in Supabase; active annual access is required for premium content and cloud writes. Existing history remains readable after expiry. There is no external analytics service. Parent audio is stored privately per authenticated account. Network access is required for uncached assets and categories. The app reports load/storage errors and supports retry.
...
`/preorder` is a parent-facing registration page: Rp39,000 annually (Rp55,000 reference price), 365 days from activation, with manual renewal. Registration is unpaid; verified administrators activate or renew access after checking payment. Automatic payment collection, billing and automatic renewal are not implemented. Four-language speech remains device-dependent. Ten introductory Islamic activities are optional in parent settings; they do not provide Quran recitation audio or a complete ritual manual. Share metadata is configured for this page.
...
The external mirror is `Oksana3301/LittleFingerActivities`. The complete initial source and all artwork were uploaded successfully on 21 September 2026; subsequent source updates are mirrored without rewriting existing history. The repository is public, including the worksheet bank. Site access checks do not protect a public GitHub copy.


## Customer account release (21 September 2026)

The new account journey is at `/register`, `/login`, `/verify-email`, `/forgot-password`, `/reset-password`, `/account`, and `/account/admin`. Email delivery flows remain explicitly closed until transactional SMTP, the exact callback URL and real verification/recovery delivery are verified. The original preorder interest form remains available. `CUSTOMER_EMAIL_READY` defaults to `false`; never enable it based solely on a successful database or Auth settings check.

Premium answers/rounds live under `content/`, not `public/`. The client catalogue contains covers only. Keep generators writing to `content/worksheets/`; never copy those bundles back to public assets. Guest access includes the introduction plus six named previews.

See [deployment and account operation notes](docs/SUPABASE_CONNECTION.md) and [launch checklist](docs/launch/OPERATIONS.md) for configuration, test coverage and the remaining owner setup.
